                    ...
                    if(myUserInfo && myUserInfo?.id > 0){
                        if(Number(myRoomResult.blind) * 2 <= Number(myUserInfo[`${myRoomResult.balanceType.toLowerCase()}Balance`])){
                            let myUserUpdateResult = await User.increment({
                                [`${myRoomResult.balanceType.toLowerCase()}Balance`]: -Number(myRoomResult.blind) * 2
                            },{
                                where: {userId}
                            })
                                                        ...
                                                        try {
                                                            ...
                                                            if(myUserUpdateResult && myUserUpdateResult[0] && myUserUpdateResult[0][1]){
                                                                ...
                                                                try {
                                                                    ...
                                                                    if(userTransactionUpdate && userTransactionUpdate.id){
                                                                        try {
                                                                            let resultGetAllConnectedList = await ConnectedList.findAll({attributes: ['socketId']})
                                                                            if(resultGetAllConnectedList && resultGetAllConnectedList?.length){ 
...
module.exports =  handleBlindTwoExe;