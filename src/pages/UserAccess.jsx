...
import { useActiveSingleAccountMutation, useDeleteSingleUserMutation, useDisabledSingleAccountMutation, useGetAllUserQuery, useUpdateDesignationMutation } from '../App/features/auth/api';
...
const UserAccess = () => {
  ...
  const userDebounce = _.debounce(handleSubmit, 1000);
  ...
  useEffect(()=>{  
    ...
    if(!isLoading && isSuccess && !isError){
      if(data && data?.id > 0){ 
        ...
        setRole('');  
        toast({
          title: "Successfully designation updated!",
          duration: 4000,
          isClosable: true,
          status: 'success'
        })
  ...
  const {data: adminData, isSuccess: adminIsSuccess} = useGetAllUserQuery();
