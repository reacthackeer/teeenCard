...
import { useGetSingleUserPlayingHistoryQuery } from '../App/features/playingHistory/copyApi';
...
const handleExploreHistory = (id) => {
    navigate('/playing-history/'+id);
}
...
const {data, isLoading, isError, isSuccess} = useGetSingleUserPlayingHistoryQuery({userId, page: currentPage},{refetchOnMountOrArgChange: true});
...
if(isSuccess && !isError && !isLoading){
    ...
    content = (
        ...
        {filteredTransactions().map((transaction, index) => (
            ...
            <VStack align="stretch">
                <Text>{transaction.balanceType} ({transaction.winnerId === userId ? 'Win' : 'Lose'})</Text>
                <Button
                    onClick={()=> handleExploreHistory(transaction.id)}
                >Explore</Button>
            </VStack>
            </HStack>
        ))}
    ...
    content = <div className='loading__container'>
                    <Heading fontSize={'large'}>No playing history founded!</Heading>
